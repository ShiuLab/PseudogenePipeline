...
#Updated on July 30, 2012 to take chunks from the left to right of contigs in Br
#   instead of the middle of the contig.
"""Algorithm for At:
...
2) Go through the BrSeqLst and make a dict of key: size val: [seq,]
...
#for cases where the randomly generated size was found in the Br dict (or where a smaller Br contig was chosen)
def BrRrEqual(size, dictx, outLst):
    #remove the seq from the val and put the seq in the outLst
    seq = dictx[size].pop(0)
    outLst.append(seq)
    
    #if val is now empty, remove the key from the dict
    if dictx[size] == []:
        dictx.pop(size)

#for cases where the randomly geneated size was not found in the Br dict,
#but a bigger Br contig can be used instead
def BrRrPlus(RrSize, BrSize, dictx, outLst):    
    ...
    #if the 2 new seqs are longer than 100bp, put them in the BrDict 
    if len(newSeqR) >= 100:
    ...
    #if the val is now empty, remove the key from the Br Dict
    if dictx[BrSize] == []:
        dictx.pop(BrSize)
...
#for Br
    ...
    #Go through the BrSeqLst and make a dict of key: size val: [seq,]
    BrSeqDict = {}
    for seq in BrSeqLst:
        size = len(seq)
    ...
    while BrSeqDict != {}:
        ...
        if randSize in BrSeqDict:
            BrRrEqual(randSize, BrSeqDict, outLst)
        else:
            cnt = 1
            #loop through to work with the closest contig in Br which has a
            #larger size
            isDone = False
            while (randSize + cnt) <= max(BrSeqDict):
                if (randSize + cnt) in BrSeqDict:
                    BrRrPlus(randSize, (randSize + cnt), BrSeqDict, outLst)
                    isDone = True
                    break
                cnt += 1
            #if no larger contig is found, count down until you find the closest
            #smaller contig
            cnt = 1
            if isDone == False:
                while (randSize - cnt) >= 100:  #probably not necissary, but prevents an infinite loop.  100 is because no contig should be smaller than 100bp in the first place
                    if (randSize - cnt) in BrSeqDict:
                        BrRrEqual((randSize - cnt), BrSeqDict, outLst)
                        break
                    cnt += 1
        if not bigCnt % 100:
            print bigCnt
