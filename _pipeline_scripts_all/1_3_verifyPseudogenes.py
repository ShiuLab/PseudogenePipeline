...
5)  Go through inGff and import names and coords of proteins into a dictionary
...
inSize = open(sys.argv[4])        #The genomic fasta .size file                                       (.fa.size)                      
inGff = open(sys.argv[5])         #The input 4col gff file with protein coordinates                   (.4col)
...
for line in inGff:
    if not line.startswith("#"):
        lineLst = line.split("\t")
        SaveIntoDict(lineLst[0], (lineLst[2], lineLst[3][:-1]), gffDict)

#6) Iterate through questionable pseudogene names dict
cnt = 0
for key in quesPseuDict:
    val = quesPseuDict[key]
    #a) Determine Yleft and Ylight
    prot = key.split(";")[0]#.split("|")[0]
    try:
        gval = gffDict[prot]
    except:
        cnt += 1
        #print gffDict
        print "Key error ", prot, " ", cnt
        #sys.exit()
        continue

    #fr is relative to the protein on its contig
    fr = ForwardOrReverse(gval[0][0], gval[0][1]) #starts with 0 because it's always a 1 member list because of the way SaveIntoDict works
    pseuProt = tuple(key.split(";")[2].split(":")[0].split("-")) #pseudogene coords on the protein
    Yl = (int(pseuProt[0]) - 1) * 3

    if fr == "F":
        Yr = int(gval[0][1]) - ((int(pseuProt[1]) * 3) + int(gval[0][0]))
    elif fr == "R":
        Yr = int(gval[0][0]) - ((int(pseuProt[1]) * 3) + int(gval[0][1]))
    else:
        print "***FR error 3***"
