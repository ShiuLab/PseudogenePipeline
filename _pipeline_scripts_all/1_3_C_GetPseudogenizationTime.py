    ...
    if not line.startswith("#"):
        ...
        if brName.startswith("Bra") and psName.startswith("PsBr") and brName in orthoDict:
            ...
            for atName in atNames:        
                ...
                psBrKa = lineLst[4]; psBrKs = lineLst[5]; psBrKaKs = lineLst[6]
...
for line in psWGD:
    if not line.startswith("#"):
        lineLst = line.strip().split("\t")
        brName = "_".join(lineLst[0].split("_")[1:])
    ...
    for group in psDict[psName]:
        ...
        if key in inOutDict and key in divDict and brName in psList:
            ...
            newLine = "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % \
                      (psName, brName, atName, one, two, Nns, t1, t, ratio)
            out.write(newLine)
