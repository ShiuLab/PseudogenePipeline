...
class blast_util:
    ...
    def verify_clade(self,bdir,fasta,fasta2,d,clades,DEBUG):
        ...
        print("Iterate through %i clades:" % len(list(cdict.keys())))
        ...
        for i in cdict:
            ...
            while inl2 != "":
                L = inl2.split("\t")
                N = "%s %s" % (L[0],L[1])
                if "%s %s" % (L[0],L[1]) not in mdict and \
                   "%s %s" % (L[1],L[0]) not in mdict:
    ...
    def batch_sw(self,program,pairs,fasta1,fasta2,bdir,outflag=0,wdir="",ev=""):
        ...
        for i in gpairs:
            ...
            missed = 0
            if i[0] not in fd1 or i[1] not in fd1:          
