...
#
# IMPORTS
import sys, os, time, math, ParseBlast
...
def help():
    ...
    print("   b_data    The type of tblastn data: tabular or blastplus")
...
def GFFto4col(gff,feature,f_index,c_index,start,stop,name,truncate):
    ...
    Return:
      gene_4col: the name of the 4 column file which is written to the current folder
    """
        ...
        if not line.startswith("#"):
            ...
            if split_line[f_index-1] == feature:
                ...
                if truncate == "true":
                    ...
                    split_sequence_information = \
                                sequence_information.strip().split(";")
                    split_sequence_information = \
                                [_f for _f in split_sequence_information if _f] 
                    # remove empty spaces
                    ...
                    split_sequence_information = [_f for _f in split_sequence_information if _f] # remove empty spaces
                    for item in split_sequence_information:
                        split_item = item.strip().split("=")
                        info_dict[split_item[0]] = split_item[1]
                    gene_name = info_dict["ID"]
...
def check_parameters(parameter_file):
    ...
    for i in inl:
        ...
        L = i.strip().split("=")
        if len(L) != 2:
            print(" parameter format problem:",L)
            p_misformat = 1
        else:     
            P[L[0]] = L[1]

    # Check if required values are missing
    required = ["b_out","p_seq","g_seq","p_codes","b_data","b_filter",
    ...
    if not P["b_data"] == "tabular":
        ...
        blast_out = P["b_out"]
        if P["b_data"] == "blastplus":
            os.system(
                "python %s/ParseBlast.py -f parse_align2_blastPlus -blast %s" %\
                    (P["p_codes"], blast_out))
            P["b_out"] = blast_out + ".mod"
        else:
            print("Error: Unknown blast format -", P["b_data"])
            b_unkformat = 1
...
def check_seq_names(p_seq, g_seq, b_out):
    ...
    p_cov = float(len(b_out_p_names_set.intersection(p_names_set)))/float(len(b_out_p_names_set))
    c_cov = float(len(b_out_c_names_set.intersection(g_names_set)))/float(len(b_out_c_names_set))

    original_protein = {}
    original_contig = {}
    if p_cov < 0.99 or c_cov < 0.99: # If there are missing names in the blast ouput
...
def main():
    ...
    os.system("python %s/FastaManager.py -f get_sizes -fasta %s" % (scriptDir, fasta))
    os.system("python %s/FastaManager.py -f get_sizes -fasta %s" % (scriptDir, prot))
    ...
    print("  GFF file:", base + ".4col.true.fa.ref %s.4col.true.RMfilt.hiConf.cdnm.gff")

    if mismatch_status == "truncated":
        print("\nRewriting Pseudogene GFF with original names... [" + FormatTime(time.time() - start_timer) + "]")
