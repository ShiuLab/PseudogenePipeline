...
ncbl2_openlib(char *name, int ldnaseq)
{
  ...
  if (ldnaseq==SEQT_PROT) {	/* read a protein database */
    newname(lname,name,AA_LIST_EXT,(int)sizeof(lname));
...
/*    else aa_btof[i]=aascii['X']; */
    ...
    newname(lname,name,NT_LIST_EXT,(int)sizeof(lname));
  ...
  /* check first for list name */
  max_oid = oid_seqs = 0;
  oid_list = NULL;
  if ((ifile = fopen(lname,"r"))!=NULL) {

    if ((bp = strrchr(name,SLASH_CHAR))!=NULL) {
      *bp = '\0';
      strncpy(db_dir,name,sizeof(db_dir));
      strncat(db_dir,SLASH_STR,sizeof(db_dir)-strlen(db_dir)-1);
      *bp = SLASH_CHAR;
    }
    else {
      db_dir[0]='\0';
	...
	pref_db = 14;
      ...
      strncpy(tname,db_dir,sizeof(tname));
      strncat(tname,msk_name, sizeof(tname));
      strncpy(msk_name, tname, sizeof(msk_name));

      strncpy(tname,db_dir,sizeof(tname));
      strncat(tname,dname, sizeof(tname));
      strncpy(dname,tname,sizeof(dname));
    ...
    else {	/* we had a .msk file, but there are no oid's in it.
		   allocate an m_fptr and return it empty */
	...
	return NULL;
      ...
      m_fptr->max_oid = 0;
      m_fptr->oid_seqs = 0;
      m_fptr->oid_list = (unsigned int *)calloc(1,sizeof(int));
      m_fptr->pref_db= -1;
...
*get_asn_seqid(unsigned char *abp,
  ...
  *db = db_type;
  ...
  switch(db_type) {
  case 0: 
    abp = get_asn_local_id(abp+2, acc);
    break;
  case 1:
  case 2:
